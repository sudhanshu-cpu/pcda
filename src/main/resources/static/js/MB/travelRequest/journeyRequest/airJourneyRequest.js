...
function showRail()
{
	...
	$("#railRequestXform").show();
	
	$("#airRequestXform").hide();
	 
	
	$("#mixedRequestXform").hide();
...
function showReturnRail()
{
	$("#returnTravelMode").val(0);
	
	
	$("#railReturnRequestXform").show();
	
	$("#airReturnRequestXform").hide();
	
	$("#mixedReturnRequestXform").hide();
	$("#returnMixedPreferenceDiv").hide();
...
function showReturnMixed()
{
	if(confirm("Rescheduling is not possible in Mixed Mode as of now, without cancellation of the whole journey for one side. Please make sure the date of journey is confirmed before booking") == true)
	{ 
		
		$('#returnMixedPreference').val(-1);
		
		$("#returnTravelMode").val(2);
	    
	    $("#railReturnRequestXform").hide();
	    
		$("#airReturnRequestXform").hide();
		
		$("#returnMixedPreferenceDiv").show();
		$("#mixedReturnRequestXform").hide();
		
		$("#returnSearchDiv").hide();	
...
function setOrgDestStnByTrRule(msg,ramBaanCheck)
{
	...
	$.each(msg.familyDetail, function(i,obj){
		    ...
		    if($('#removeFrmList'+i).is(":checked"))
		    {
				...
				var k=0;
				$.each(obj.yearWise, function(ix,objYear){
				...
				if(formDAndGYear==YearWiseFormDAndG)
					{
						...
						{
							if(travelTypeText=='FORMD' || travelTypeText=='FORMG' || travelTypeText=='OTHERS')
							{
	...
	if(ramBaanCheck!="true")
	{
       ...
       for(var i=0;i<oldReq.length;i++)
       {
        ...
        if(!(orgOptions.indexOf('AnyStation') > -1))
        {
        	if(travelTypeText.indexOf('LTC-AI')>-1  && taggCheck!="true" )
        	{
             setReturnOrgStnValue(destStn)
             
            if(trRuleID=='TR100234'){
      	 	$("#returnOrgStnDropDown").show(); 
      	 	 
      	 	}else{
          	$("#returnOrgStnDropDown").hide();
          	 }                        
	       	}else{
	       		$("#returnOrgStnDropDown").show();       
	       	}
...
function hideReturnRoundTrip()
{
	...
	var isBothModeRequestTaken=$('#returnIsBothModeRequestTaken').val();
	var isRailRequestOnwardExist = $("#returnIsRailRequestOnwardExist").val();		
	var isRailRequestReturnExist = $("#returnIsRailRequestReturnExist").val();		
	var isAirRequestOnwardExist = $("#returnIsAirRequestOnwardExist").val();		
	var isAirRequestReturnExist = $("#returnIsAirRequestReturnExist").val();		
	var isAirOneWayExist=$('#returnIsAirOneWayExist').val();
	...
	if(travelTypeText.indexOf('LTC')>-1 && travelMode==1)
	{
		...
		if($('#returnAirJrnyTypeDiv1')!=null)$('#returnAirJrnyTypeDiv1').hide();
		if($('#returnAirJrnyTypeDiv2')!=null)$('#returnAirJrnyTypeDiv2').hide();
		if($('#returnReturnDateRow')!=null)$('#returnReturnDateRow').hide();
...
function airListPopup()
{
	...
	if(result!=false)
	{
		...
		//tejpurItanagarAirportmethod(origin,destination);
		...
		if(origin=='TEZ'){
			$('#oldAirport').val('TEZ');
			origin='HGI';			
			$('#origin').val('Donyi Polo Airport,Itanagar,Assam,India(HGI)');
		...
		if(destination=='TEZ'){
			$('#oldAirport').val('TEZ');
			destination='HGI';
			$('#destination').val('Donyi Polo Airport,Itanagar,Assam,India(HGI)');
...
function airReturnListPopup()
{
	...
	if(result!=false)
	{
		...
		if(origin=='TEZ'){
			...
			origin='HGI';
			$('#returnOrigin').val('Donyi Polo Airport,Itanagar,Assam,India(HGI)');
			originName='Donyi Polo Airport, Itanagar,Assam,India';
		...
		if(destination=='TEZ'){
			$('#returnOldAirport').val('TEZ');
			destination='HGI';
			$('#returnDestination').val('Donyi Polo Airport,Itanagar,Assam,India(HGI)');
			destinationName='Donyi Polo Airport, Itanagar,Assam,India';
		}
			}
			/*TEZPUR AIRPORT CODE */
...
if(serpov=='ATT'){
	...
	$.ajax({
		type: "POST",
		url:$("#context_path").val()+"mb/jrnyATTFareRule",
...
if(serpov=='BL'){
		...
		$.ajax({
		type: "POST",
		url:$("#context_path").val()+"mb/jrnyBLFareRule",
