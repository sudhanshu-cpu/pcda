...
function doGenerate(event) {
	var result = validateDateFilterTT();
	if (result) {
...
function validateDateFilterTT() {

	var from = $("#frmBookingDt").val();
	var to = $("#toBookingDt").val();
	
	var tranFrom=$("#frmTransactionDt").val();
	var tranTo=$("#toTransactionDt").val();
...
if (tranFrom != "" && tranTo == "") {
	 ...
	 if(tranFrom != "" && tranTo != ""){
	tranFrom = tranFrom.split("/");
	tranTo = tranTo.split("/");
   var sTDate = new Date(tranFrom[2]+","+(tranFrom[1])+","+tranFrom[0]);
   
  	var eTDate = new Date(tranTo[2]+","+(tranTo[1])+","+tranTo[0]);
  	if(sTDate > eTDate )
  	{
  	   alert("Transaction To Date Should Greater Than Transaction From Date");
  	  $("#toTransactionDt").focus();
  	   return false;
  	}
	}
...
function submitAcknowledgementData() {
	...
	if (Voucher_No == '') {
		...
		if (emptyInput != '') {
			...
			//	$("#NoOfCheckBox").val(checkBoxArr.length);
