package com.pcda.cda.transactionsettlementair.outstandingvouchergeneration.service;
...
import com.pcda.cda.transactionsettlementair.outstandingvouchergeneration.model.AccountOfficeResponse;
import com.pcda.cda.transactionsettlementair.outstandingvouchergeneration.model.AfterVoucherGenCdaResponse;
import com.pcda.cda.transactionsettlementair.outstandingvouchergeneration.model.GenerateVoucherCdaResponse;
...
import com.pcda.cda.transactionsettlementair.outstandingvouchergeneration.model.PostOutVouchGenCdaChildModel;
...
public class OutVoucherCdaService {

@Autowired

private RestTemplate restTemplate;
	
 public List<GetOutVoucherGenCdaDataModel>	getGenerateReportData(OutVoucherGenCdaReqModel genReqModel){
	 
	 List<GetOutVoucherGenCdaDataModel>  modelList = new ArrayList<>();
	
	 DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class, " genReqModel "+genReqModel);
	...
	"/getVoucherGenerateData?fromBookingDate={fromBookingDate}&toBookingDate={toBookingDate}&bookingId={bookingId}&txnId={txnId}&serviceType={serviceType}&personalNo={personalNo}&travelType={travelType}&accountOffice={accountOffice}&statusType={statusType}&unitOffice={unitOffice}&serviceProvider={serviceProvider}";
	 ...
	 GenerateVoucherCdaResponse response = restTemplate.getForObject(url, GenerateVoucherCdaResponse.class, params);
	 ...
	 DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class, "getGenerateReportData  Response List "+modelList.size());
	}
	catch(Exception e) {
 ...
 // create and download csv
	...
	public void createCSVDownload(PrintWriter printWriter , List<GetOutVoucherGenCdaDataModel> modelList) {
		...
		buffer.append("K3");
		 ...
		 for(GetOutVoucherGenCdaDataModel model : modelList) {
...
//popup
	...
	public  List<PopUpParamCdaModel> voucherGenerationSummaryDataInXML(HttpServletRequest request){
		 
		  List<PopUpParamCdaModel>  paramModelList = new ArrayList<>(); 
		try {
		  ...
		  DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class, " personalNo "+personalNo+" transactionId "+transactionId
			 ...
			 if(null!=personalNoValues && personalNoValues.length>0)
			 {
				...
				for(int i=0;i<personalNoValues.length;i++)
				 	{
					PopUpParamCdaModel model =  new PopUpParamCdaModel();
		...
		DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class, " paramModelList "+paramModelList.size());
...
public PostOutStandVoucherGenCdaModel getPopUpModelList(List<String> txnsId,OutVoucherGenCdaReqModel reqModel){
	DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class, " txnsId "+txnsId);
	
	List<GetOutVoucherGenCdaDataModel> filteredModelList=new ArrayList<>();
	
	
	
	List<GetOutVoucherGenCdaDataModel> modelDataList = getGenerateReportData(reqModel);
	
	for(String txn : txnsId ) {
		
		for(GetOutVoucherGenCdaDataModel model : modelDataList) {
	...
	List<PostOutVouchGenCdaChildModel> childModelList = new ArrayList<>(); 
	
if(filteredModelList !=null) {
 
	for(GetOutVoucherGenCdaDataModel child : filteredModelList) {
		
		PostOutVouchGenCdaChildModel postChildModel = new PostOutVouchGenCdaChildModel();
	...
	PostOutStandVoucherGenCdaModel postParent = new PostOutStandVoucherGenCdaModel();
	...
	DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class, " postParent "+postParent);
	return postParent;
}
...
public AfterVoucherGenCdaResponse sendDataForsave(PostOutStandVoucherGenCdaModel postParent) {
	AfterVoucherGenCdaResponse  response =new AfterVoucherGenCdaResponse();
	try {
		
       response = restTemplate.postForObject(PcdaConstant.AIR_SERVICE_VOUCHER+"/getGenerateVoucher", postParent, AfterVoucherGenCdaResponse.class);
...
public Map<String,String> getAccountOfficeListMap(String officeType,String groupId){
	DODLog.info(LogConstant.CDA_VOUCHER_LOG_FILE,OutVoucherCdaService.class,"getAccountOfficeListMap() officeType" + officeType + " | groupId ::"+groupId);
	AccountOfficeResponse response=null;
	Map<String,String> responseMap=new HashMap<>();
	try {
		 String url = PcdaConstant.OUTSTANDING_VOUCHER_GENERATION_URL+
					"/getMappedAccountOffice/";
		 response = restTemplate.getForObject(url+officeType+"/"+groupId, AccountOfficeResponse.class);
