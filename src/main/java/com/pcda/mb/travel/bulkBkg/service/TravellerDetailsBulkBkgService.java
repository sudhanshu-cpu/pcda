...
import com.pcda.mb.travel.bulkBkg.model.PreventProcessBulkBkgResponse;
import com.pcda.mb.travel.bulkBkg.model.PreventProcessDetailsBulkBkg;
...
@Service
public class TravellerDetailsBulkBkgService {
	...
	public TravellerDetailsBulkBkgModel getTravellerDetails(Optional<String> userAlias, HttpServletRequest request) {
		...
		TravellerDetailsBulkBkgModel  travellerDetails=new TravellerDetailsBulkBkgModel();
		try {
		...
		Optional<Level> level=  levelServices.getLevel(travler.getLevelId());
		 ...
		 // Validating Level Update
		   ...
		   if(dodServices.isPresent()){
			...
			DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+" Permission Type::"+permissionType+"  Personal Number::"+userAlias);
			
			if(permissionType == 1){
			 String levelId=travler.getLevelId();
			 DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+" LevelId::"+levelId);
			...
			synchronized (this)
			{
				 ...
				 DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+" :: groupId = "+groupId);
				 List<PreventProcessDetailsBulkBkg> processDetails=getProcessData(personalUserId,processName);
				 
			 
				 if(processDetails==null||processDetails.isEmpty())
				 {
					 DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+" :: inside if = processDetails==null");
					 ...
					 ResponseEntity<PreventProcessBulkBkgResponse> processResponse= template.postForEntity(PcdaConstant.PREVENT_PROCESS_BASE_URL+"/save", preventProcessDetails, PreventProcessBulkBkgResponse.class);
					
					 DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+":New Row Inserted For"+processName+":-="+processResponse.getStatusCode().value());
				 ...
				 else
				 {
					 ...
					 PreventProcessDetailsBulkBkg preventProcessDetails=processDetails.get(0);
				 	
					DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+":GET_PERSONAL_INFO Process Record Exist:-");
					DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+":ID:-"+preventProcessDetails.getProcessId()+"||PROCESS_NAME="+preventProcessDetails.getProcessName()+"||PROCESS_STATE="+preventProcessDetails.getProcessStatus());
					DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+":PERSONAL_ID:-"+preventProcessDetails.getPersonalId()+"||PERSONAL_NO="+preventProcessDetails.getPersonalNumber()+"||UNIT_USER_LOGIN_ID="+preventProcessDetails.getUnitLoginId());
					DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, logStr+":GROUP_ID:-"+preventProcessDetails.getGroupId()+"||ENTRY_TIME="+preventProcessDetails.getEntryTime()+"||EXIT_TIME="+preventProcessDetails.getExitTime());
...
private List<PreventProcessDetailsBulkBkg> getProcessData(BigInteger personalUserId, String processName) {
		
		List<PreventProcessDetailsBulkBkg> processDetails=new ArrayList<>();
		try {
		ResponseEntity<PreventProcessBulkBkgResponse> responseEntity = template.exchange(
				PcdaConstant.PREVENT_PROCESS_BASE_URL + "/getProcess/"+personalUserId+"/"+processName, HttpMethod.GET, null,
				new ParameterizedTypeReference<PreventProcessBulkBkgResponse>() {});
		
		PreventProcessBulkBkgResponse processResponse = responseEntity.getBody();
		...
		if (null != processResponse && (processResponse.getErrorCode() == 200) && null != processResponse.getResponseList()) {
			processDetails.addAll(processResponse.getResponseList());
			 ...
			 DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, personalUserId+" :: EXCEPTION  = "+processName);
			DODLog.printStackTrace(e, JourneyRequestAjaxService.class, LogConstant.JOURNEY_REQUEST_LOG);
					}
		DODLog.info(LogConstant.JOURNEY_REQUEST_LOG, JourneyRequestAjaxService.class, " processDetails :"+processDetails);	
		return processDetails;
	}

private void setAllTravelTypeXml(TravellerDetailsBulkBkgModel travellerDetails) {
...
private void setAllCivilianTravelTypeXml(TravellerDetailsBulkBkgModel travellerDetails) {
